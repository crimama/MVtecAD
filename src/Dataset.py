    ...
    def __init_aug__(self,Augmentation):
        if Augmentation == None:
            augmentation = transforms.Compose([
                                                ...
                                                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) # Chnnel, height, width 
                                            ])
        else: 
...
class DataInit:
    ...
    def test_load(self):
        ...
        if self.cls == 'toothbrush':
            test_img_labels = np.array(test_img_labels)
            test_img_labels = np.where(test_img_labels==test_label_unique['good'],test_img_labels,999)
            test_img_labels = np.where(test_img_labels==999,test_img_labels,0)
            test_img_labels = np.where(test_img_labels==0,test_img_labels,1)
        else: 
            test_img_labels = np.array(test_img_labels)
